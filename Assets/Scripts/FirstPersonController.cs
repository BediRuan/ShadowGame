...
public class FirstPersonController : MonoBehaviour
{
    ...
    public AudioSource fastStepAudioSource;
    public AudioClip footstepClip;          // 脚步声的音频剪辑
    public AudioClip fastStepClip;
    #endregion
    ...
    private void Update()
    {
        ...
        if (isSprinting && isGrounded) // 优先处理奔跑状态
        {
            if (!fastStepAudioSource.isPlaying) // 如果没有播放奔跑脚步声
            {
                fastStepAudioSource.clip = fastStepClip; // 确保设置为奔跑音效
                fastStepAudioSource.loop = false;
                fastStepAudioSource.Play();
            }

            if (footstepAudioSource.isPlaying) // 停止播放步行脚步声
            {
                footstepAudioSource.Stop();
            }
        }
        else if (isWalking && isGrounded) // 其次处理步行状态
        {
            if (!footstepAudioSource.isPlaying) // 如果没有播放步行脚步声
            {
                footstepAudioSource.clip = footstepClip; // 确保设置为步行音效
                footstepAudioSource.loop = false;
                footstepAudioSource.Play();
            }

            if (fastStepAudioSource.isPlaying) // 停止播放奔跑脚步声
            {
                fastStepAudioSource.Stop();
            }
        }
        else // 停止所有脚步声
        {
            ...
            if (fastStepAudioSource.isPlaying)
            {
                fastStepAudioSource.Stop();
            }
        }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("Footstep Audio Settings", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 });

        fpc.footstepAudioSource = (AudioSource)EditorGUILayout.ObjectField(new GUIContent("Audio Source", "AudioSource for footstep sounds"), fpc.footstepAudioSource, typeof(AudioSource), true);
        fpc.fastStepAudioSource = (AudioSource)EditorGUILayout.ObjectField(new GUIContent("Audio Source", "AudioSource for fastStep sounds"), fpc.fastStepAudioSource, typeof(AudioSource), true);
        fpc.footstepClip = (AudioClip)EditorGUILayout.ObjectField(new GUIContent("Footstep Clip", "Audio clip for footstep sounds"), fpc.footstepClip, typeof(AudioClip), true);
        fpc.fastStepClip = (AudioClip)EditorGUILayout.ObjectField(new GUIContent("FastStep Clip", "Audio clip for Fast step sounds"), fpc.fastStepClip, typeof(AudioClip), true);
